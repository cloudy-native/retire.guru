...
describe('prepareChartData', () => {
  test('transforms scenario data to chart data format', () => {
    ...
    const scenarioData = [
      {
        ...
        withdrawalRate: "4.80",
        // New real-value fields
        inflationIndex: 1,
        ssMonthlyReal: 2000,
        requiredFrom401kMonthlyReal: 2000,
        totalMonthlyIncomeReal: 4000,
        startingBalanceReal: 500000,
        yearlyReturnReal: 25000,
        endingBalanceReal: 500000,
      },
      {
        year: currentYear + 1,
        ssMonthly: 2040,
        ...
        // New real-value fields
        inflationIndex: 1.03,
        ssMonthlyReal: 1980.58,
        requiredFrom401kMonthlyReal: 2000.00,
        totalMonthlyIncomeReal: 3980.58,
        startingBalanceReal: 485436.89,
        yearlyReturnReal: 24271.84,
        endingBalanceReal: 436893.20,
      }
    ];
    ...
    expect(result[0]).toEqual({ year: currentYear, Balance: 500000, BalanceReal: 500000 });
    expect(result[1]).toEqual({ year: currentYear + 1, Balance: 450000, BalanceReal: 436893.20 });
  });
  ...
  test('handles years beyond scenario data with zero balance', () => {
    ...
    expect(result[0]).toEqual({ year: currentYear, Balance: 500000, BalanceReal: 500000 });
    expect(result[1]).toEqual({ year: currentYear + 1, Balance: 0, BalanceReal: 0 });
    expect(result[2]).toEqual({ year: currentYear + 2, Balance: 0, BalanceReal: 0 });
  });
});